...
class StudJobSupabaseUser extends BaseAuthUser {
  StudJobSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> studJobSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = StudJobSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
